...
export class CoreApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getCoreAPIVersions(options?: Configuration): Promise<RequestContext> {
...
export class CoreApiResponseProcessor {
     ...
     */
     public async getCoreAPIVersions(response: ResponseContext): Promise<IoK8sApimachineryPkgApisMetaV1APIVersions > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: IoK8sApimachineryPkgApisMetaV1APIVersions = ObjectSerializer.deserialize(
