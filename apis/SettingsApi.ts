...
export class SettingsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getSettingsAPIGroup(options?: Configuration): Promise<RequestContext> {
		let config = options || this.configuration;
		
		// Path Params
    	const localVarPath = '/apis/settings.k8s.io/';
...
export class SettingsApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to getSettingsAPIGroup
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getSettingsAPIGroup(response: ResponseContext): Promise<IoK8sApimachineryPkgApisMetaV1APIGroup > {
